...
function fetchTeachers() {
  fetch("http://localhost:8080/teachers") // Hace un GET al localhost
...
function handleFormSubmit(event) {
  ...
  if (id) {
      fetch(`http://localhost:8080/teachers?id=${id}`, {
          method: "PUT",
...
function deleteTeacher(id) {
  ...
  if (confirm("Are you sure you want to delete this teacher?")) {
      fetch(`http://localhost:8080/teachers?id=${id}`, { method: "DELETE" })
